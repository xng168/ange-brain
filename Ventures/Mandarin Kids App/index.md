...
- **Stage:** **M0 built (2026-07-18).** Expo project scaffolded at `C:\Users\Angela\Projects\mandarin-kids-app` (Expo SDK 57 / RN 0.86, TypeScript), build kit installed, 20-card placeholder pack, calm "Heritage Words" home screen. iOS bundle verified (582 modules, clean export) + TypeScript typechecks. Git: branch `main`, commit `3f05df5`. **Next:** Angela tests it on her iPhone via Expo Go, then M1 (pack engine). The full build kit was reconstructed from the claude.ai web session and imported into `build-kit/` (see Documents).
