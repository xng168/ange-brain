...
- 2026-07-13 (bug check, commit `4ecc702`): independent review of Phase 1 found and fixed 2 defects before they could bite: (1) missing Supabase env vars crashed the whole page on first form submit (module-level throw, no error boundary) — now fails gracefully with a friendly form message, which matters because deploying to Vercel without env vars *looks* fine until someone submits; (2) emails weren't lowercased, so the same address with different capitalisation created duplicate rows and broke the "already on the list" reply. Verified: build passes, lint clean, RLS insert-only policy correct, `.env.local` properly gitignored with the example tracked, disclaimer footer present. Known-and-accepted for Phase 1: no rate limiting on the public waitlist insert (revisit at Phase 7 polish), `shadcn` CLI listed as a runtime dependency (harmless cleanup).
